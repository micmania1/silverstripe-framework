...
class FolderTest extends SapphireTest {
	...
	public function setUp() {
		...
		foreach($fileIDs as $fileID) {
			...
			$filename = $filesystem->makeAbsolute($file->Filename);
			$fh = fopen($filename, "w");
	...
	public function testFindOrMake() {
		...
		$this->assertTrue($this->getFilesystem()->has($path), 'File');
	...
	public function testFindOrMakeFolderThenMove() {
		...
		$this->assertEquals($file1->Filename, 'FileTest-folder2/FileTest-folder1/File1.txt',
	...
	public function testRenameFolderAndCheckTheFile() {
		...
		$file1 = DataObject::get_by_id('File', $this->idFromFixture('File', 'file1-folder1'), false);
		$this->assertFileExists($file1->getFullPath());
		$this->assertEquals($file1->Filename, 'FileTest-folder1-changed/File1.txt',
	...
	public function testGetRelativePath() {
		...
		$this->assertEquals('FileTest-folder1/', $rootfolder->getRelativePath(), 'Folder in assets/');
	}
	...
	public function testGetFullPath() {
		$rootfolder = $this->objFromFixture('Folder', 'folder1');
		$this->assertEquals($this->getFilesystem()->getBasePath() . '/FileTest-folder1', $rootfolder->getFullPath(),
	...
	public function testDeleteAlsoRemovesFilesystem() {
		...
		$folder = Folder::find_or_make($path);
		$absolute = $this->getFilesystem()->makeAbsolute($path);
		$this->assertFileExists($absolute);

		$folder->delete();

		$this->assertFileNotExists($absolute);
	}
	...
	public function testDeleteAlsoRemovesSubfoldersInDatabaseAndFilesystem() {
		...
		$this->assertFileNotExists($this->getFilesystem()->makeAbsolute($path));
		$this->assertFileNotExists($this->getFilesystem()->makeAbsolute($subfolderPath),
			'Subfolder removed from filesystem');
	...
	public function testSyncedChildren() {
		$filesystem = $this->getFilesystem();
		$filesystem->createDir('FolderTest/sync');
		...
		foreach($files as $file) {
			$path = $filesystem->makeAbsolute('FolderTest/sync/' . $file);
			$fh = fopen($path, "w");
			fwrite($fh, 'test');
			fclose($fh);
		}
		...
		foreach($folders as $folder) {
			$filesystem->createDir('FolderTest/sync/' . $folder);
		}

		$folder = Folder::find_or_make('FolderTest/sync');
		$result = $folder->syncChildren();

		$this->assertEquals(7, $result['skipped']);
		$this->assertEquals(2, $result['added']);

		// folder with a path of _testsync should exist

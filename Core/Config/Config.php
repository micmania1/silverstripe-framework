...
class Config {
	...
	public function get($class, $name, $sourceOptions = 0) {
		...
		$includeExtensions = ($sourceOptions & self::EXCLUDE_EXTRA_SOURCES) != self::EXCLUDE_EXTRA_SOURCES;
		if ($includeExtensions) {
			...
			if(is_array($extensions)) {
				foreach($extensions as $extensionClass) {
					...
					if (!is_array($extraConfig) && !is_array($value)) {
        ...
        if (
            ...
            && (($sourceOptions & self::FIRST_SET) != self::FIRST_SET || $value == null)
			...
			while($parent = get_parent_class($parent)) {
				...
				// We can't merge into non-arrays
				...
				// If we don't have a value yet, we cast to an array so we can merge up
				if(is_null($value)) {
					$value = array();
				}
				...
				// Merge the parent value into the current value. If the current value
				// is not set, then we just take the parent value
	...
	public function update($class, $name, $val) {
		// $eol = \SilverStripe\Control\Director::is_cli() ? PHP_EOL : '<br />';
		// echo 'Config->update() called on "' . $class . '.' . $name . '"' . $eol;
		return;
	...
	public function remove($class, $name, $key = null, $value = null) {
		// $eol = \SilverStripe\Control\Director::is_cli() ? PHP_EOL : '<br />';
		// $msg = sprintf('Config->remove() is being called on %s.%s%s', $class, $name, $eol);
		// echo $msg;
		return;
